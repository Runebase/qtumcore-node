...
var RunebaseRPC = require('runebased-rpc');
...
/**
 * Provides a friendly event driven API to runebased in Node.js. Manages starting and
 * stopping runebased as a child process for application support, as well as connecting
 * to multiple runebased processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Runebase(options) {
  if (!(this instanceof Runebase)) {
    return new Runebase(options);
  }
...
util.inherits(Runebase, Service);

Runebase.dependencies = [];

Runebase.DEFAULT_MAX_TXIDS = 1000;
Runebase.DEFAULT_MAX_HISTORY = 50;
Runebase.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Runebase.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Runebase.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Runebase.DEFAULT_SPAWN_RESTART_TIME = 5000;
Runebase.DEFAULT_SPAWN_STOP_TIME = 10000;
Runebase.DEFAULT_TRY_ALL_INTERVAL = 1000;
Runebase.DEFAULT_REINDEX_INTERVAL = 10000;
Runebase.DEFAULT_START_RETRY_INTERVAL = 5000;
Runebase.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Runebase.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Runebase.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Runebase.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Runebase.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Runebase.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Runebase.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Runebase.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Runebase.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Runebase.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Runebase.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Runebase.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Runebase.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Runebase.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Runebase.prototype._initCaches = function() {
...
Runebase.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Runebase.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Runebase.prototype.getPublishEvents = function() {
  return [
    {
...
Runebase.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'runebased/' + name, 'total:', this.subscriptions[name].length);
};

Runebase.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'runebased/' + name, 'total:', this.subscriptions[name].length);
};


Runebase.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
    ...
    log.info(emitter.remoteAddress, 'subscribe:', 'runebased/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Runebase.prototype.unsubscribeBalance = function(emitter, addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'runebased/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

Runebase.prototype.unsubscribeBalanceAll = function(emitter) {
...
Runebase.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'runebased/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Runebase.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'runebased/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Runebase.prototype.unsubscribeAddressAll = function(emitter) {
...
Runebase.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Runebase.DEFAULT_CONFIG_SETTINGS;
...
Runebase.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
Runebase.prototype._expandRelativeDatadir = function() {
...
Runebase.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in runebased config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in runebased config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in runebased config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './runebase.conf');

  log.info('Using runebase config file:', configPath);
...
Runebase.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of runebasecore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of runebasecore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of runebasecore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to runebased from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from runebased. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from runebased. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that runebased is undergoing a reindex. ' +
             ...
             'of runebasecore-node services will start.');
    node._reindex = true;
  }
};

Runebase.prototype._resetCaches = function() {
...
Runebase.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Runebase.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Runebase.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Runebase Daemon Ready');
          callback();
        });
      });
...
Runebase.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8332
  };
...
Runebase.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/runebase.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/runebase.conf';
    }
  }
  return networkPath;
};

Runebase.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
Runebase.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('runebased/hashblock', message.toString('hex'));
    }
  }

};

Runebase.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Runebase.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Runebase Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Runebase.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Runebase.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Runebase.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('runebased/addressbalance', {
                    address: address,
                    txid: txid,
...
Runebase.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('runebased/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Runebase.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('runebased/rawtransaction', message.toString('hex'));
    }
...
Runebase.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Runebase.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Runebase.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Runebase.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Runebase.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Runebase Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Runebase.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Runebase.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Runebase.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/runebased.pid';
...
Runebase.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting runebase process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Runebase process unexpectedly exited with code:', code);
        log.warn('Restarting runebase child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Runebase process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn runebased.'));
      }
...
Runebase.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to runebased.'));
    }
 ...
 */
Runebase.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Runebase configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Runebase.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Runebase.prototype.syncPercentage = function(callback) {
  var self = this;
...
Runebase.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, runebasecore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Runebase.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Runebase.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Runebase.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Runebase.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Runebase.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in runebasecore is most recent to less recent
 ...
 * @param {Function} callback
 */
Runebase.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Runebase.prototype._getConfirmationsDetail = function(transaction) {
...
Runebase.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Runebase.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Runebase.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A runebasecoin transaction id
 * @param {Function} callback
 */
Runebase.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Runebase.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Runebase.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Runebase.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Runebase.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Runebase.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Runebase.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Runebase.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Runebase.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
Runebase.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Runebase.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Runebase.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
Runebase.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Runebase.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Runebase.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Runebase.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Runebase.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Runebasecore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Runebase.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Runebase.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Runebase.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Runebase.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Runebase.prototype.getInfo = function(callback) {
...
Runebase.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Runebase.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('runebased spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('runebased process did not exit'));
      }
 ...
 * @param {Function} callback
 */
Runebase.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Runebase.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Runebase.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
Runebase.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
Runebase.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
Runebase.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
Runebase.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
Runebase.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
Runebase.prototype.getStakingInfo = function(callback) {
    var self = this,
