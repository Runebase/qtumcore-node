...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for runebased is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and runebasecore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed runebasecore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './runebasecore-node.json');
  ...
  node.on('ready', function() {
    log.info('Runebasecore Node ready');
  });
...
function loadModule(req, service) {
  try {
    // first try in the built-in runebasecore-node services directory
    ...
    if (servicePackage.runebasecoreNode) {
      serviceModule = service.name + '/' + servicePackage.runebasecoreNode;
    }
