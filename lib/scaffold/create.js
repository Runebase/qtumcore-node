...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'runebasecore-lib': 'https://github.com/runebase/runebasecore-lib.git#master',
    'runebasecore-node': 'https://github.com/runebase/runebasecore-node.git#master'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/runebasecore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the runebasecore-node directory and configuration
