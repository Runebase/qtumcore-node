...
var Transaction = runebasecore.Transaction;
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/runebase.conf')));
var RunebaseService = proxyquire('../../lib/services/runebased', {
	fs: {
		readFileSync: readFileSync
	}
});
var defaultRunebaseConf = fs.readFileSync(path.resolve(__dirname, '../data/default.runebase.conf'), 'utf8');

describe('Runebase Service', function() {
	...
	var baseConfig = {
		node: {
			network: runebasecore.Networks.testnet
		},
		spawn: {
			datadir: 'testdir',
			exec: 'testpath'
	...
	describe('@constructor', function() {
		it('will create an instance', function() {
			var runebased = new RunebaseService(baseConfig);
			should.exist(runebased);
		});
		it('will create an instance without `new`', function() {
			var runebased = RunebaseService(baseConfig);
			should.exist(runebased);
		});
		it('will init caches', function() {
			var runebased = new RunebaseService(baseConfig);

			should.exist(runebased.utxosCache);
			should.exist(runebased.txidsCache);
			should.exist(runebased.balanceCache);
			should.exist(runebased.summaryCache);
			should.exist(runebased.transactionDetailedCache);
			should.exist(runebased.accountInfo);

			should.exist(runebased.transactionCache);
			should.exist(runebased.rawTransactionCache);
			should.exist(runebased.rawJsonTransactionCache);
			should.exist(runebased.blockCache);
			should.exist(runebased.blockJsonCache);
			should.exist(runebased.blockSubsidyCache);
			should.exist(runebased.rawBlockCache);
			should.exist(runebased.blockHeaderCache);
			should.exist(runebased.zmqKnownTransactions);
			should.exist(runebased.zmqKnownBlocks);
			should.exist(runebased.lastTip);
			should.exist(runebased.lastTipTimeout);
			should.equal(runebased.dgpInfoCache, null, 'should be null');
			should.equal(runebased.miningInfoCache, null, 'should be null');
			should.equal(runebased.stakingInfoCache, null, 'should be null');

			// limits
			should.equal(runebased.maxTxids, 1000);
			should.equal(runebased.maxTransactionHistory, 50);
			should.equal(runebased.maxAddressesQuery, 10000);
			should.equal(runebased.shutdownTimeout, 15000);

			// spawn restart setting
			should.equal(runebased.spawnRestartTime, 5000);
			should.equal(runebased.spawnStopTime, 10000);

			// try all interval
			should.equal(runebased.tryAllInterval, 1000);
			should.equal(runebased.startRetryInterval, 5000);

			// rpc limits
			should.equal(runebased.transactionConcurrency, 5);

			// sync progress level when zmq subscribes to events
			should.equal(runebased.zmqSubscribeProgress, 0.9999);
		});
		it('will init clients', function() {
			var runebased = new RunebaseService(baseConfig);
			runebased.nodes.should.deep.equal([]);
			runebased.nodesIndex.should.equal(0);
			runebased.nodes.push({ client: sinon.stub() });
			should.exist(runebased.client);
		});
		it('will set subscriptions', function() {
			var runebased = new RunebaseService(baseConfig);
			runebased.subscriptions.should.deep.equal({
				address: {},
				rawtransaction: [],
				hashblock: [],
				balance: {},
	...
	describe('#_initDefaults', function() {
		it('will set transaction concurrency', function() {
			var runebased = new RunebaseService(baseConfig);
			runebased._initDefaults({ transactionConcurrency: 10 });
			runebased.transactionConcurrency.should.equal(10);
			runebased._initDefaults({});
			runebased.transactionConcurrency.should.equal(5);
		});
	});

	describe('@dependencies', function() {
		it('will have no dependencies', function() {
			RunebaseService.dependencies.should.deep.equal([]);
		});
	});
	...
	describe('#getAPIMethods', function() {
		it('will return spec', function() {
			var runebased = new RunebaseService(baseConfig);
			var methods = runebased.getAPIMethods();
	...
	describe('#getPublishEvents', function() {
		it('will return spec', function() {
			var runebased = new RunebaseService(baseConfig);
			var events = runebased.getPublishEvents();
			should.exist(events);
			events.length.should.equal(4);

			events[0].name.should.equal('runebased/rawtransaction');
			events[0].scope.should.equal(runebased);
			events[0].subscribe.should.be.a('function');
			events[0].unsubscribe.should.be.a('function');

			events[1].name.should.equal('runebased/hashblock');
			events[1].scope.should.equal(runebased);
			events[1].subscribe.should.be.a('function');
			events[1].unsubscribe.should.be.a('function');

			events[2].name.should.equal('runebased/addresstxid');
			events[2].scope.should.equal(runebased);
			events[2].subscribe.should.be.a('function');
			events[2].unsubscribe.should.be.a('function');

			events[3].name.should.equal('runebased/addressbalance');
			events[3].scope.should.equal(runebased);
		...
		it('will call subscribe/unsubscribe with correct args', function() {
			var runebased = new RunebaseService(baseConfig);
			runebased.subscribe = sinon.stub();
			runebased.unsubscribe = sinon.stub();
			var events = runebased.getPublishEvents();

			events[0].subscribe('test');
			runebased.subscribe.args[0][0].should.equal('rawtransaction');
			runebased.subscribe.args[0][1].should.equal('test');

			events[0].unsubscribe('test');
			runebased.unsubscribe.args[0][0].should.equal('rawtransaction');
			runebased.unsubscribe.args[0][1].should.equal('test');

			events[1].subscribe('test');
			runebased.subscribe.args[1][0].should.equal('hashblock');
			runebased.subscribe.args[1][1].should.equal('test');

			events[1].unsubscribe('test');
			runebased.unsubscribe.args[1][0].should.equal('hashblock');
			runebased.unsubscribe.args[1][1].should.equal('test');
		});
	});
	...
	describe('#subscribe', function() {
		...
		it('will push to subscriptions', function() {
			var runebased = new RunebaseService(baseConfig);
			var emitter = {};
			runebased.subscribe('hashblock', emitter);
			runebased.subscriptions.hashblock[0].should.equal(emitter);

			var emitter2 = {};
			runebased.subscribe('rawtransaction', emitter2);
			runebased.subscriptions.rawtransaction[0].should.equal(emitter2);
		});
	});
	...
	describe('#unsubscribe', function() {
		...
		it('will remove item from subscriptions', function() {
			...
			var emitter4 = {};
			var emitter5 = {};
			runebased.subscribe('hashblock', emitter1);
			runebased.subscribe('hashblock', emitter2);
			runebased.subscribe('hashblock', emitter3);
			runebased.subscribe('hashblock', emitter4);
			runebased.subscribe('hashblock', emitter5);
			runebased.subscriptions.hashblock.length.should.equal(5);

			runebased.unsubscribe('hashblock', emitter3);
			runebased.subscriptions.hashblock.length.should.equal(4);
			runebased.subscriptions.hashblock[0].should.equal(emitter1);
			runebased.subscriptions.hashblock[1].should.equal(emitter2);
			runebased.subscriptions.hashblock[2].should.equal(emitter4);
			runebased.subscriptions.hashblock[3].should.equal(emitter5);
		});
		it('will not remove item an already unsubscribed item', function() {
			var runebased = new RunebaseService(baseConfig);
			var emitter1 = {};
			var emitter3 = {};
			runebased.subscriptions.hashblock = [emitter1];
			runebased.unsubscribe('hashblock', emitter3);
			runebased.subscriptions.hashblock.length.should.equal(1);
			runebased.subscriptions.hashblock[0].should.equal(emitter1);
		});
	});
	...
	describe('#subscribeAddress', function() {
		...
		it('will not an invalid address', function() {
			...
			runebased.subscribeAddress(emitter, ['invalidaddress']);
			should.not.exist(runebased.subscriptions.address['invalidaddress']);
		});
		it('will add a valid address', function() {
			var runebased = new RunebaseService(baseConfig);
			var emitter = new EventEmitter();
			runebased.subscribeAddress(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will handle multiple address subscribers', function() {
			var runebased = new RunebaseService(baseConfig);
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			runebased.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			runebased.subscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will not add the same emitter twice', function() {
			...
			runebased.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddress', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			runebased.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			runebased.unsubscribeAddress(emitter1);
			runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			runebased.unsubscribeAddress(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			runebased.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
		});
		it('will remove empty addresses', function() {
			...
			runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			runebased.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			runebased.unsubscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will unsubscribe emitter for all addresses', function() {
			...
			var emitter2 = new EventEmitter();
			runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			runebased.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(runebased, 'unsubscribeAddressAll');
			runebased.unsubscribeAddress(emitter1);
			runebased.unsubscribeAddressAll.callCount.should.equal(1);
			runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			runebased.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddressAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			runebased.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			runebased.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			runebased.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			runebased.unsubscribeAddressAll(emitter1);
			runebased.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			runebased.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			runebased.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(runebased.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
		});
	});
	...
	describe('#_getDefaultConfig', function() {
		it('will generate config file from defaults', function() {
			var runebased = new RunebaseService(baseConfig);
			var config = runebased._getDefaultConfig();
			config.should.equal(defaultRunebaseConf);
		});
	});
	...
	describe('#_loadSpawnConfiguration', function() {
		...
		it('will parse a runebase.conf file', function() {
			var TestRunebase = proxyquire('../../lib/services/runebased', {
				fs: {
					readFileSync: readFileSync,
			...
			var runebased = new TestRunebase(baseConfig);
			runebased.options.spawn.datadir = '/tmp/.runebase';

			sinon.spy(runebased, '_expandRelativeDatadir');
			sinon.spy(runebased, '_checkConfigIndexes');
			sinon.spy(runebased, '_getDefaultConf')
			sinon.spy(runebased, '_getNetworkConfigPath');
			sinon.spy(runebased, '_parseBitcoinConf');

			var node = {};
			runebased._loadSpawnConfiguration(node);

			runebased._expandRelativeDatadir.callCount.should.equal(1);
			runebased._checkConfigIndexes.callCount.should.equal(1);
			runebased._getDefaultConf.callCount.should.equal(1);
			runebased._getNetworkConfigPath.callCount.should.equal(1);
			runebased._parseBitcoinConf.callCount.should.equal(2);

			should.exist(runebased.spawn.config);
			runebased.spawn.config.should.deep.equal({
				addressindex: 1,
				checkblocks: 144,
				dbcache: 8192,
		...
		it('will expand relative datadir to absolute path', function() {
			...
			var config = {
				node: {
					network: runebasecore.Networks.testnet,
					configPath: '/tmp/.runebasecore/runebasecore-node.json'
				},
				spawn: {
					datadir: './data',
					exec: 'testpath'
			...
			var runebased = new TestRunebase(config);
			...
			runebased.options.spawn.datadir = './data';
			var node = {};
			runebased._loadSpawnConfiguration(node);
			...
			runebased.options.spawn.datadir.should.equal('/tmp/.runebasecore/data');
		});
		it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
			var TestRunebase = proxyquire('../../lib/services/runebased', {
				fs: {
					readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badrunebase.conf')),
					existsSync: sinon.stub().returns(true),
				},
				mkdirp: {
					sync: sinon.stub()
			...
			var runebased = new TestRunebase(baseConfig);
			(function() {
				runebased._loadSpawnConfiguration({ datadir: './test' });
			}).should.throw(runebasecore.errors.InvalidState);
		});
	...
	describe('#_checkConfigIndexes', function() {
		...
		it('should warn the user if txindex isn\'t set to 1 in the runebase.conf file', function() {
			var runebased = new RunebaseService(baseConfig);
			var config = {
				txindex: 0,
				addressindex: 1,
			...
			var node = {};
			(function() {
				runebased._checkConfigIndexes(config, node);
			}).should.throw('"txindex" option');
		});
		it('should warn the user if addressindex isn\'t set to 1 in the runebase.conf file', function() {
			var runebased = new RunebaseService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 0,
		...
		it('should warn the user if spentindex isn\'t set to 1 in the runebase.conf file', function() {
			var runebased = new RunebaseService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if server isn\'t set to 1 in the runebase.conf file', function() {
			var runebased = new RunebaseService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if reindex is set to 1 in the runebase.conf file', function() {
			var runebased = new RunebaseService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
	...
	describe('#_resetCaches', function() {
		it('will reset LRU caches', function() {
			...
			for (var i = 0; i < 10; i++) {
				...
				runebased.transactionDetailedCache.set(keys[i], {});
				runebased.utxosCache.set(keys[i], {});
				runebased.accountInfo.set(keys[i], {});
				runebased.txidsCache.set(keys[i], {});
				runebased.balanceCache.set(keys[i], {});
				runebased.summaryCache.set(keys[i], {});
				runebased.blockOverviewCache.set(keys[i], {});
				runebased.blockJsonCache.set(keys[i], {});
			}
			runebased._resetCaches();
			should.equal(runebased.transactionDetailedCache.get(keys[0]), undefined);
			should.equal(runebased.utxosCache.get(keys[0]), undefined);
			should.equal(runebased.accountInfo.get(keys[0]), undefined);
			should.equal(runebased.txidsCache.get(keys[0]), undefined);
			should.equal(runebased.balanceCache.get(keys[0]), undefined);
			should.equal(runebased.summaryCache.get(keys[0]), undefined);
			should.equal(runebased.blockOverviewCache.get(keys[0]), undefined);
			should.equal(runebased.blockJsonCache.get(keys[0]), undefined);
	...
	describe('#_tryAllClients', function() {
		it('will retry for each node client', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased.tryAllInterval = 1;
			runebased.nodes.push({
				client: {
			...
			runebased._tryAllClients(function(client, next) {
				client.getInfo(next);
			}, function(err) {
				if (err) {
					return done(err);
				}
				runebased.nodes[0].client.getInfo.callCount.should.equal(1);
				runebased.nodes[1].client.getInfo.callCount.should.equal(1);
				runebased.nodes[2].client.getInfo.callCount.should.equal(1);
				done();
			});
		});
		it('will start using the current node index (round-robin)', function(done) {
			...
			runebased._tryAllClients(function(client, next) {
				...
				runebased.nodes[1].client.getInfo.callCount.should.equal(1);
				runebased.nodes[2].client.getInfo.callCount.should.equal(1);
				runebased.nodesIndex.should.equal(2);
				done();
			});
		});
	...
	describe('#_wrapRPCError', function() {
		it('will convert runebased-rpc error object into JavaScript error', function() {
			var runebased = new RunebaseService(baseConfig);
			var error = runebased._wrapRPCError({ message: 'Test error', code: -1 });
	...
	describe('#_initChain', function() {
		...
		it('will set height and genesis buffer', function(done) {
			...
			runebased.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
			runebased.nodes.push({
				client: {
			...
			runebased._initChain(function() {
				log.info.callCount.should.equal(1);
				runebased.getRawBlock.callCount.should.equal(1);
				runebased.getRawBlock.args[0][0].should.equal('genesishash');
				runebased.height.should.equal(5000);
				runebased.genesisBuffer.should.equal(genesisBuffer);
				done();
			});
		});
		it('it will handle error from getBestBlockHash', function(done) {
			...
			runebased._initChain(function(err) {
		...
		it('it will handle error from getRawBlock', function(done) {
			...
			runebased.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
			runebased._initChain(function(err) {
				err.should.be.instanceOf(Error);
				getBestBlockHash.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
	...
	describe('#_getDefaultConf', function() {
		afterEach(function() {
			runebasecore.Networks.disableRegtest();
			baseConfig.node.network = runebasecore.Networks.testnet;
		});
		it('will get default rpc port for livenet', function() {
			var config = {
				node: {
					network: runebasecore.Networks.livenet
				},
				spawn: {
					datadir: 'testdir',
					exec: 'testpath'
			...
			var runebased = new RunebaseService(config);
			runebased._getDefaultConf().rpcport.should.equal(8332);
		});
		it('will get default rpc port for testnet', function() {
			...
			var runebased = new RunebaseService(config);
			runebased._getDefaultConf().rpcport.should.equal(18332);
		});
	...
	describe('#_getNetworkConfigPath', function() {
		...
		it('will get default config path for livenet', function() {
			...
			var runebased = new RunebaseService(config);
			should.equal(runebased._getNetworkConfigPath(), undefined);
		});
		it('will get default rpc port for testnet', function() {
			var config = {
				node: {
			...
			var runebased = new RunebaseService(config);
			runebased._getNetworkConfigPath().should.equal('testnet3/runebase.conf');
		});
		it('will get default rpc port for regtest', function() {
			runebasecore.Networks.enableRegtest();
			var config = {
			...
			var runebased = new RunebaseService(config);
			runebased._getNetworkConfigPath().should.equal('regtest/runebase.conf');
		});
	});
	...
	describe('#_getNetworkOption', function() {
		...
		it('return --testnet for testnet', function() {
			var runebased = new RunebaseService(baseConfig);
			runebased.node.network = runebasecore.Networks.testnet;
			runebased._getNetworkOption().should.equal('--testnet');
		});
		it('return --regtest for testnet', function() {
			var runebased = new RunebaseService(baseConfig);
			runebased.node.network = runebasecore.Networks.testnet;
			runebasecore.Networks.enableRegtest();
			runebased._getNetworkOption().should.equal('--regtest');
		});
		it('return undefined for livenet', function() {
			var runebased = new RunebaseService(baseConfig);
			runebased.node.network = runebasecore.Networks.livenet;
			runebasecore.Networks.enableRegtest();
			should.equal(runebased._getNetworkOption(), undefined);
		});
	});
	...
	describe('#_zmqBlockHandler', function() {
		it('will emit block', function(done) {
			...
			runebased._rapidProtectedUpdateTip = sinon.stub();
			runebased.on('block', function(block) {
				block.should.equal(message);
				done();
			});
			runebased._zmqBlockHandler(node, message);
			runebased._rapidProtectedUpdateTip.callCount.should.equal(1);
		});
		it('will not emit same block twice', function(done) {
			...
			var message = new Buffer('76843f1dfe455347d897e757ff5ff51a16a0f62b5d02a659c5680392de3a2b89', 'hex');
			runebased._rapidProtectedUpdateTip = sinon.stub();
			sinon.spy(runebased, 'emit');
			runebased.on('block', function(block) {
				block.should.equal(message);
				runebased.emit.callCount.should.equal(1);
				runebased._rapidProtectedUpdateTip.callCount.should.equal(1);
				done();
			});
			runebased._zmqBlockHandler(node, message);
		...
		it('will call function to update tip', function() {
			...
			runebased._zmqBlockHandler(node, message);
			runebased._rapidProtectedUpdateTip.callCount.should.equal(1);
			runebased._rapidProtectedUpdateTip.args[0][0].should.equal(node);
			runebased._rapidProtectedUpdateTip.args[0][1].should.equal(message);
		});
		it('will emit to subscribers', function(done) {
			...
			sinon.spy(runebased, 'emit');
			var emitter = new EventEmitter();
			runebased.subscriptions.hashblock.push(emitter);
			emitter.on('runebased/hashblock', function(blockHash) {
				runebased._rapidProtectedUpdateTip.callCount.should.equal(1);
				runebased.emit.callCount.should.equal(1);
	...
	describe('#_rapidProtectedUpdateTip', function() {
		it('will limit tip updates with rapid calls', function(done) {
			...
			var count = 0;
			function repeat() {
				runebased._rapidProtectedUpdateTip(node, message);
				count++;
				if (count < 50) {
					repeat();
				}
	...
	describe('#_updateTip', function() {
		...
		it('log and emit rpc error from get block', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased.syncPercentage = sinon.stub();
			sinon.spy(runebased, '_resetCaches');
			runebased.on('error', function(err) {
				err.code.should.equal(-1);
				err.message.should.equal('Test error');
				node.client.getBlock.callCount.should.equal(1);
				runebased._resetCaches.callCount.should.equal(1);
			...
			runebased._updateTip(node, message);
		});
		it('emit synced if percentage is 100', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
			runebased.on('synced', function() {
				runebased.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
				done();
			});
			var node = {
		...
		it('NOT emit synced if percentage is less than 100', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
			runebased.on('synced', function() {
				runebased.syncPercentage.callCount.should.equal(0);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('log and emit error from syncPercentage', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			runebased.on('error', function(err) {
				log.error.callCount.should.equal(1);
				runebased.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('reset caches and set height', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased.syncPercentage = sinon.stub();
			runebased._resetCaches = sinon.stub();
			runebased.on('tip', function(height) {
				runebased._resetCaches.callCount.should.equal(1);
				height.should.equal(10);
				runebased.height.should.equal(10);
				done();
			});
			var node = {
				client: {
	...
	describe('#_getAddressesFromTransaction', function() {
		it('will get results using runebasecore.Transaction', function() {
			var runebased = new RunebaseService(baseConfig);
			var wif = 'cVTiUNavxbCzuoaQboDcgEhKYHhgXRaeQ8ZRoYkVXE9mxT1TcSrc';
			var privkey = runebasecore.PrivateKey.fromWIF(wif);
			var inputAddress = privkey.toAddress(runebasecore.Networks.testnet);
			var outputAddress = runebasecore.Address('qMDYc9oXQK19dmLSCQNs5okRt8z4XeMr1X');
			var tx = runebasecore.Transaction();
			tx.from({
				txid: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				outputIndex: 0,
				script: runebasecore.Script(inputAddress),
				address: inputAddress.toString(),
				satoshis: 5000000000
			});
			tx.to(outputAddress, 5000000000);
			tx.sign(privkey);
			var addresses = runebased._getAddressesFromTransaction(tx);
		...
		it('will handle non-standard script types', function() {
			var runebased = new RunebaseService(baseConfig);
			var tx = runebasecore.Transaction();
			tx.addInput(runebasecore.Transaction.Input({
				prevTxId: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				script: runebasecore.Script('OP_TRUE'),
				outputIndex: 1,
				output: {
					script: runebasecore.Script('OP_TRUE'),
					satoshis: 5000000000
			...
			tx.addOutput(runebasecore.Transaction.Output({
				script: runebasecore.Script('OP_TRUE'),
				satoshis: 5000000000
			}));
			var addresses = runebased._getAddressesFromTransaction(tx);
		...
		it('will return unique values', function() {
			var runebased = new RunebaseService(baseConfig);
			var tx = runebasecore.Transaction();
			var address = runebasecore.Address('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX');
			tx.addOutput(runebasecore.Transaction.Output({
				script: runebasecore.Script(address),
				satoshis: 5000000000
			}));
			tx.addOutput(runebasecore.Transaction.Output({
				script: runebasecore.Script(address),
	...
	describe('#_notifyAddressTxidSubscribers', function() {
		it('will emit event if matching addresses', function(done) {
			...
			runebased._getAddressesFromTransaction = sinon.stub().returns([address]);
			var emitter = new EventEmitter();
			runebased.subscriptions.address[address] = [emitter];
			var txid = 'a9f5d9dc39bfff65c12d8a7a8263f57cc187d4c6cc62815d65186726bbc5ecb5';
			var transaction = {};
			emitter.on('runebased/addresstxid', function(data) {
			...
			runebased._notifyAddressTxidSubscribers(txid, transaction);
		});
	...
	describe('#_zmqTransactionHandler', function() {
		it('will emit to subscribers', function(done) {
			...
			var emitter = new EventEmitter();
			runebased.subscriptions.rawtransaction.push(emitter);
			emitter.on('runebased/rawtransaction', function(hex) {
				hex.should.be.a('string');
				hex.should.equal(expectedBuffer.toString('hex'));
				done();
			});
			var node = {};
			runebased._zmqTransactionHandler(node, expectedBuffer);
		});
		it('will NOT emit to subscribers more than once for the same tx', function(done) {
			...
			sinon.spy(runebased, 'emit');
			runebased.subscriptions.rawtransaction.push(emitter);
			emitter.on('runebased/rawtransaction', function() {
				done();
			});
			var node = {};
			runebased._zmqTransactionHandler(node, expectedBuffer);
		...
		it('will emit "tx" event', function(done) {
			var runebased = new RunebaseService(baseConfig);
			var expectedBuffer = new Buffer(txhex, 'hex');
			runebased.on('tx', function(buffer) {
	...
	describe('#_checkSyncedAndSubscribeZmqEvents', function() {
		...
		it('log errors, update tip and subscribe to zmq events', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased._updateTip = sinon.stub();
			runebased._subscribeZmqEvents = sinon.stub();
			var blockEvents = 0;
			runebased.on('block', function() {
				blockEvents++;
			});
			var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
			...
			runebased._checkSyncedAndSubscribeZmqEvents(node);
			setTimeout(function() {
				...
				runebased._updateTip.callCount.should.equal(11);
				runebased._subscribeZmqEvents.callCount.should.equal(1);
				done();
			}, 200);
		});
	...
	describe('#_subscribeZmqEvents', function() {
		it('will call subscribe on zmq socket', function() {
			...
			runebased._subscribeZmqEvents(node);
		...
		it('will call relevant handler for rawtx topics', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased._zmqTransactionHandler = sinon.stub();
			var node = {
			...
			node.zmqSubSocket.on('message', function() {
				runebased._zmqTransactionHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will call relevant handler for hashblock topics', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased._zmqBlockHandler = sinon.stub();
			var node = {
				zmqSubSocket: new EventEmitter()
			};
			node.zmqSubSocket.subscribe = sinon.stub();
			runebased._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				runebased._zmqBlockHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will ignore unknown topic types', function(done) {
			...
			runebased._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				runebased._zmqBlockHandler.callCount.should.equal(0);
				runebased._zmqTransactionHandler.callCount.should.equal(0);
				done();
			});
	...
	describe('#_initZmqSubSocket', function() {
		it('will setup zmq socket', function() {
			...
			var runebased = new RunebaseService(baseConfig);
			var node = {};
			runebased._initZmqSubSocket(node, 'url');
	...
	describe('#_checkReindex', function() {
		...
		it('give error from client getblockchaininfo', function(done) {
			...
			runebased._checkReindex(node, function(err) {
				should.exist(err);
		...
		it('will wait until sync is 100 percent', function(done) {
			...
			runebased._checkReindex(node, function() {
	...
	describe('#_loadTipFromNode', function() {
		...
		it('will give rpc error from client getbestblockhash', function(done) {
			...
			runebased._loadTipFromNode(node, function(err) {
				err.should.be.instanceof(Error);
		...
		it('will set height and emit tip', function(done) {
			...
			runebased.on('tip', function(height) {
				...
				runebased.height.should.equal(100);
				done();
			});
			runebased._loadTipFromNode(node, function(err) {
				if (err) {
	...
	describe('#_stopSpawnedProcess', function() {
		...
		it('it will kill process and resume', function(done) {
			...
			var TestRunebaseService = proxyquire('../../lib/services/runebased', {
				fs: {
					readFile: readFile
				}
			});
			var runebased = new TestRunebaseService(baseConfig);
			runebased.spawnStopTime = 1;
			runebased._process = {};
			runebased._process.kill = sinon.stub();
			runebased._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
				runebased._process.kill.callCount.should.equal(1);
				log.warn.callCount.should.equal(1);
				done();
			});
		});
		it('it will attempt to kill process and resume', function(done) {
			...
			error2.code = 'ESRCH';
			runebased._process.kill = sinon.stub().throws(error2);
			runebased._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
	...
	describe('#_spawnChildProcess', function() {
		...
		it('will give error from spawn config', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased._loadSpawnConfiguration = sinon.stub();
			runebased._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
			runebased._spawnChildProcess(function(err) {
				runebased._loadSpawnConfiguration.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give error from stopSpawnedBitcoin', function() {
			var runebased = new RunebaseService(baseConfig);
			runebased._loadSpawnConfiguration = sinon.stub();
			runebased._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
			runebased._spawnChildProcess(function(err) {
				runebased._loadSpawnConfiguration.callCount.should.equal(1);
				runebased._stopSpawnedBitcoin.callCount.should.equal(1);
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
			});
		});
		it('will exit spawn if shutdown', function() {
			...
			var runebased = new TestRunebaseService(config);
			runebased.spawn = {};
			runebased._loadSpawnConfiguration = sinon.stub();
			runebased._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
			runebased.node.stopping = true;
			runebased._spawnChildProcess(function(err) {
				err.should.be.instanceOf(Error);
		...
		it('will include network with spawn command and init zmq/rpc on node', function(done) {
			...
			runebased._loadSpawnConfiguration = sinon.stub();
			runebased.spawn = {};
			runebased.spawn.exec = 'testexec';
			runebased.spawn.configPath = 'testdir/runebase.conf';
			runebased.spawn.datadir = 'testdir';
			runebased.spawn.config = {};
			runebased.spawn.config.rpcport = 20001;
			runebased.spawn.config.rpcuser = 'runebase';
			runebased.spawn.config.rpcpassword = 'password';
			runebased.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

			runebased._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			runebased._initZmqSubSocket = sinon.stub();
			runebased._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			runebased._checkReindex = sinon.stub().callsArgWith(1, null);
			runebased._spawnChildProcess(function(err, node) {
				should.not.exist(err);
				spawn.callCount.should.equal(1);
				spawn.args[0][0].should.equal('testexec');
				spawn.args[0][1].should.deep.equal([
					'--conf=testdir/runebase.conf',
					'--datadir=testdir',
					'--testnet'
				]);
				...
				runebased._loadTipFromNode.callCount.should.equal(1);
				runebased._initZmqSubSocket.callCount.should.equal(1);
				should.exist(runebased._initZmqSubSocket.args[0][0].client);
				runebased._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
				runebased._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
				should.exist(runebased._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
				should.exist(node);
				should.exist(node.client);
				done();
			});
		});
		it('will respawn runebased spawned process', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestRunebaseService = proxyquire('../../lib/services/runebased', {
				fs: {
			...
			runebased._loadSpawnConfiguration = sinon.stub();
			runebased.spawn = {};
			runebased.spawn.exec = 'runebased';
			runebased.spawn.datadir = '/tmp/runebase';
			runebased.spawn.configPath = '/tmp/runebase/runebase.conf';
			runebased.spawn.config = {};
			runebased.spawnRestartTime = 1;
			runebased._loadTipFromNode = sinon.stub().callsArg(1);
			runebased._initZmqSubSocket = sinon.stub();
			runebased._checkReindex = sinon.stub().callsArg(1);
			runebased._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			runebased._stopSpawnedBitcoin = sinon.stub().callsArg(0);
			sinon.spy(runebased, '_spawnChildProcess');
			runebased._spawnChildProcess(function(err) {
				if (err) {
					return done(err);
				}
				process.once('exit', function() {
					setTimeout(function() {
						runebased._spawnChildProcess.callCount.should.equal(2);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will emit error during respawn', function(done) {
			...
			runebased._spawnChildProcess(function(err) {
				...
				runebased._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
				runebased.on('error', function(err) {
					err.should.be.instanceOf(Error);
					err.message.should.equal('test');
					done();
		...
		it('will NOT respawn runebased spawned process if shutting down', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestRunebaseService = proxyquire('../../lib/services/runebased', {
				fs: {
			...
			runebased._spawnChildProcess(function(err) {
				...
				process.once('exit', function() {
					setTimeout(function() {
						runebased._spawnChildProcess.callCount.should.equal(1);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will give error after 60 retries', function(done) {
			...
			var runebased = new TestRunebaseService(baseConfig);
			runebased.startRetryInterval = 1;
			runebased._loadSpawnConfiguration = sinon.stub();
			runebased.spawn = {};
			runebased.spawn.exec = 'testexec';
			runebased.spawn.configPath = 'testdir/runebase.conf';
			...
			runebased.spawn.config.rpcpassword = 'password';
			runebased.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
			runebased._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
			runebased._spawnChildProcess(function(err) {
				runebased._loadTipFromNode.callCount.should.equal(60);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give error from check reindex', function(done) {
			...
			runebased._initZmqSubSocket = sinon.stub();
			runebased._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			runebased._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
	...
	describe('#_connectProcess', function() {
		it('will give error if connecting while shutting down', function(done) {
			...
			var runebased = new RunebaseService(config);
			runebased.node.stopping = true;
			runebased.startRetryInterval = 100;
			runebased._loadTipFromNode = sinon.stub();
			runebased._connectProcess({}, function(err) {
				err.should.be.instanceof(Error);
				err.message.should.match(/Stopping while trying to connect to runebased/);
				runebased._loadTipFromNode.callCount.should.equal(0);
				done();
			});
		});
		it('will give error from loadTipFromNode after 60 retries', function(done) {
			...
			runebased.startRetryInterval = 1;
			var config = {};
			runebased._connectProcess(config, function(err) {
				err.should.be.instanceof(Error);
				runebased._loadTipFromNode.callCount.should.equal(60);
				done();
			});
		...
		it('will init zmq/rpc on node', function(done) {
			...
			runebased._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			var config = {};
			runebased._connectProcess(config, function(err, node) {
				should.not.exist(err);
				runebased._loadTipFromNode.callCount.should.equal(1);
				runebased._initZmqSubSocket.callCount.should.equal(1);
				runebased._loadTipFromNode.callCount.should.equal(1);
	...
	describe('#start', function() {
		...
		it('will give error if "spawn" and "connect" are both not configured', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased.options = {};
			runebased.start(function(err) {
				err.should.be.instanceof(Error);
				err.message.should.be.a('string').and.equal('Runebase configuration options "spawn" or "connect" are expected');
				done();
			});
		});
		...
		it('will give error from connectProcess', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
			runebased.options = {
				connect: [
					{}
				]
			...
			runebased.start(function(err) {
				runebased._connectProcess.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will push node from spawnChildProcess', function(done) {
			var runebased = new RunebaseService(baseConfig);
			var node = {};
			runebased._initChain = sinon.stub().callsArg(0);
			runebased._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
			runebased.options = {
				spawn: {}
			};
			runebased.start(function(err) {
				should.not.exist(err);
				runebased.nodes.length.should.equal(1);
				done();
			});
		});
		it('will push node from connectProcess', function(done) {
			...
			sinon.spy(runebased, '_spawnChildProcess');
			var nodes = [{}];
			runebased._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
			runebased.options = {
				connect: [
					{}
				]
			...
			runebased.start(function(err) {
				should.not.exist(err);
				runebased._initChain.callCount.should.equal(1);
				runebased._connectProcess.callCount.should.equal(1);
				runebased.nodes.length.should.equal(1);
				done();
			});
	...
	describe('#isSynced', function() {
		it('will give error from syncPercentage', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			runebased.isSynced(function(err) {
				should.exist(err);
				runebased.syncPercentage.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
		...
		it('will give "true" if percentage is 100.00', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
			runebased.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
				runebased.syncPercentage.callCount.should.equal(1);
		...
		it('will give "true" if percentage is 99.98', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
			runebased.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 99.49', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
			runebased.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 1', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
			runebased.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
	...
	describe('#syncPercentage', function() {
		it('will give rpc error', function(done) {
			...
			runebased.syncPercentage(function(err) {
				should.exist(err);
				runebased.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
		...
		it('will call client getInfo and give result', function(done) {
			...
			runebased.syncPercentage(function(err, percentage) {
				if (err) {
					return done(err);
				}
				runebased.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
	...
	describe('#_normalizeAddressArg', function() {
		it('will turn single address into array', function() {
			var runebased = new RunebaseService(baseConfig);
			var args = runebased._normalizeAddressArg('address');
			args.should.deep.equal(['address']);
		});
		it('will keep an array as an array', function() {
			var runebased = new RunebaseService(baseConfig);
			var args = runebased._normalizeAddressArg(['address one', 'address two']);
	...
	describe('#getAddressBalance', function() {
		it('will give rpc error', function(done) {
			var runebased = new RunebaseService(baseConfig);
			sinon.spy(runebased, '_normalizeAddressArg');
			runebased.nodes.push({
				client: {
			...
			var options = {};
			runebased.getAddressBalance(address, options, function(err) {
				runebased._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give balance', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			var options = {};
			runebased.getAddressBalance(address, options, function(err, data) {
				if (err) {
					return done(err);
				}
				data.balance.should.equal(10000);
				data.received.should.equal(100000);
				runebased.getAddressBalance(address, options, function(err, data2) {
					if (err) {
						return done(err);
					}
					data2.balance.should.equal(10000);
					data2.received.should.equal(100000);
					runebased._normalizeAddressArg.callCount.should.equal(2);
	...
	describe('#getAddressUnspentOutputs', function() {
		it('will give rpc error', function(done) {
			...
			runebased.getAddressUnspentOutputs(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give results from client getaddressutxos', function(done) {
			...
			var address = 'qMYemSxspKpzB2RbDunJRosQv5PJd3eEWD';
			runebased.getAddressUnspentOutputs(address, options, function(err, utxos) {
				if (err) {
					return done(err);
				}
	...
	describe('#_getBalanceFromMempool', function() {
		it('will sum satoshis', function() {
			...
			var sum = runebased._getBalanceFromMempool(deltas);
	...
	describe('#_getTxidsFromMempool', function() {
		it('will filter to txids', function() {
			...
			var txids = runebased._getTxidsFromMempool(deltas);
	...
	describe('#_getHeightRangeQuery', function() {
		it('will detect range query', function() {
			...
			var rangeQuery = runebased._getHeightRangeQuery(options);
		...
		it('will get range properties', function() {
			...
			var clone = {};
			var rangeQuery = runebased._getHeightRangeQuery(options, clone);
		...
		it('will throw error with invalid range', function() {
			...
			(function() {
				runebased._getHeightRangeQuery(options);
	...
	describe('#getAddressTxids', function() {
		it('will give error from _getHeightRangeQuery', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
			runebased.getAddressTxids('address', {}, function(err) {
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give rpc error from mempool query', function() {
			...
			sinon.spy(runebased, '_getHeightRangeQuery');
			sinon.spy(runebased, '_normalizeAddressArg');
			...
			var options = {};
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			runebased.getAddressTxids(address, options, function(err) {
				should.exist(err);
				runebased._getHeightRangeQuery.callCount.should.equal(1);
				runebased._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
			});
		});
		...
		it('will get txid results', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			runebased.getAddressTxids(address, options, function(err, txids) {
				if (err) {
					return done(err);
				}
				runebased._getHeightRangeQuery.callCount.should.equal(1);
		...
		it('will get txid results from cache', function(done) {
			...
			runebased.getAddressTxids(address, options, function(err, txids) {
				...
				runebased.getAddressTxids(address, options, function(err, txids) {
					...
					runebased._getHeightRangeQuery.callCount.should.equal(2);
					runebased._normalizeAddressArg.callCount.should.equal(2);
					getAddressTxids.callCount.should.equal(1);
					txids.should.deep.equal(expectedTxids);
					done();
		...
		it('will get txid results WITHOUT cache if rangeQuery and exclude mempool', function(done) {
			...
			runebased.getAddressTxids(address, options, function(err, txids) {
				...
				runebased.getAddressTxids(address, options, function(err, txids) {
					...
					runebased._getHeightRangeQuery.callCount.should.equal(4);
					runebased._normalizeAddressArg.callCount.should.equal(2);
		...
		it('will get txid results from cache and live mempool', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			runebased.getAddressTxids(address, { queryMempool: false }, function(err, txids) {
				if (err) {
					return done(err);
				}
				getAddressTxids.callCount.should.equal(1);
				...
				runebased.getAddressTxids(address, { queryMempool: true }, function(err, txids) {
					if (err) {
						return done(err);
					}
					getAddressTxids.callCount.should.equal(1);
					...
					runebased.getAddressTxids(address, { queryMempoolOnly: true }, function(err, txids) {
						if (err) {
							return done(err);
						}
						getAddressTxids.callCount.should.equal(1);
						...
						runebased._getHeightRangeQuery.callCount.should.equal(3);
						runebased._normalizeAddressArg.callCount.should.equal(3);
						done();
					});
				});
			});
	...
	describe('#_getConfirmationDetail', function() {
		...
		it('should get 0 confirmation', function() {
			...
			var runebased = new RunebaseService(baseConfig);
			runebased.height = 10;
			var confirmations = runebased._getConfirmationsDetail(tx);
	...
	describe('#_getAddressDetailsForInput', function() {
		it('will return if missing an address', function() {
			var runebased = new RunebaseService(baseConfig);
			var result = {};
			runebased._getAddressDetailsForInput({}, 0, result, []);
		...
		it('will only add address if it matches', function() {
			var runebased = new RunebaseService(baseConfig);
			var result = {};
			runebased._getAddressDetailsForInput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForOutput', function() {
		it('will return if missing an address', function() {
			var runebased = new RunebaseService(baseConfig);
			var result = {};
			runebased._getAddressDetailsForOutput({}, 0, result, []);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
		});
		it('will only add address if it matches', function() {
			var runebased = new RunebaseService(baseConfig);
			var result = {};
			runebased._getAddressDetailsForOutput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForTransaction', function() {
		it('will calculate details for the transaction', function(done) {
			...
			sinon.spy(runebased, '_getAddressDetailsForInput');
			sinon.spy(runebased, '_getAddressDetailsForOutput');

			var addresses = ['qey8KJ8xkyLUUqFZZwqngXX9U2q9ef6Tw4'];
			var details = runebased._getAddressDetailsForTransaction(tx, addresses);
			...
			runebased._getAddressDetailsForInput.callCount.should.equal(1);
			runebased._getAddressDetailsForOutput.callCount.should.equal(5);
			done();
		});
	});
	...
	describe('#_getAddressDetailedTransaction', function() {
		it('will get detailed transaction info', function(done) {
			...
			var runebased = new RunebaseService(baseConfig);
			sinon.spy(runebased, '_getConfirmationsDetail');
			runebased.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
			runebased.height = 300;
			var addresses = {};
			runebased._getAddressDetailsForTransaction = sinon.stub().returns({
				addresses: addresses,
				satoshis: 1000,
			});
			runebased._getAddressDetailedTransaction(txid, {}, function(err, details) {
				if (err) {
					return done(err);
				}
				details.addresses.should.equal(addresses);
				details.satoshis.should.equal(1000);
				runebased.getDetailedTransaction.callCount.should.equal(1);
				runebased._getAddressDetailsForTransaction.callCount.should.equal(1);
				runebased._getConfirmationsDetail.callCount.should.equal(1);
		...
		it('give error from _getDetailedTransaction', function(done) {
			...
			var runebased = new RunebaseService(baseConfig);
			runebased.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
			runebased._getAddressDetailedTransaction(txid, {}, function(err) {
				err.should.be.instanceof(Error);
				runebased.getDetailedTransaction.callCount.should.equal(1);
				done();
			});
	...
	describe('#_getAddressStrings', function() {
		it('will get address strings from runebasecore addresses', function() {
			var addresses = [
				runebasecore.Address('qU12Fa5RHM535kSDvywxPjCmbL7gwkQJZ6'),
				runebasecore.Address('qcSLSxN1sngCWSrKFZ6UC7ri4hhVSdq9SU'),
			];
			var runebased = new RunebaseService(baseConfig);
			var strings = runebased._getAddressStrings(addresses);
		...
		it('will give error with unknown', function() {
			...
			var runebased = new RunebaseService(baseConfig);
			(function() {
				runebased._getAddressStrings(addresses);
	...
	describe('#getAddressHistory', function() {
		...
		it('will give an error if length of addresses is too long', function(done) {
			...
			var runebased = new RunebaseService(baseConfig);
			sinon.spy(runebased, '_normalizeAddressArg');
			runebased.maxAddressesQuery = 100;
			runebased.getAddressHistory(addresses, {}, function(err) {
				should.exist(err);
				runebased._normalizeAddressArg.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" range that exceeds max size', function(done) {
			var runebased = new RunebaseService(baseConfig);
			sinon.spy(runebased, '_normalizeAddressArg');
			sinon.spy(runebased, '_getAddressStrings');
			runebased.getAddressHistory(address, { from: 0, to: 51 }, function(err) {
				should.exist(err);
				runebased._normalizeAddressArg.callCount.should.equal(1);
				runebased._getAddressStrings.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" order is reversed', function(done) {
			...
			sinon.spy(runebased, '_normalizeAddressArg');
			sinon.spy(runebased, '_getAddressStrings');
			runebased.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
			runebased.getAddressHistory(address, { from: 51, to: 0 }, function(err) {
				should.exist(err);
				runebased._normalizeAddressArg.callCount.should.equal(1);
				runebased._getAddressStrings.callCount.should.equal(1);
				err.message.match(/^\"from/);
		...
		it('will give error from _getAddressDetailedTransaction', function(done) {
			...
			sinon.spy(runebased, '_normalizeAddressArg');
			sinon.spy(runebased, '_getAddressStrings');
			runebased.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
			runebased._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
			runebased.getAddressHistory(address, {}, function(err) {
				should.exist(err);
				runebased._normalizeAddressArg.callCount.should.equal(1);
				runebased._getAddressStrings.callCount.should.equal(1);
				runebased.getAddressTxids.callCount.should.equal(1);
				runebased._getAddressDetailedTransaction.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
			});
		});
		it('give error from getAddressTxids', function(done) {
			...
			sinon.spy(runebased, '_normalizeAddressArg');
			sinon.spy(runebased, '_getAddressStrings');
			sinon.spy(runebased, 'getAddressTxids');
			runebased.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			runebased.getAddressHistory('address', {}, function(err) {
				should.exist(err);
				runebased._normalizeAddressArg.callCount.should.equal(1);
				runebased._getAddressStrings.callCount.should.equal(1);
		...
		it('will paginate', function(done) {
			...
			sinon.spy(runebased, '_normalizeAddressArg');
			sinon.spy(runebased, '_getAddressStrings');
			sinon.spy(runebased, '_paginateTxids');
			runebased._getAddressDetailedTransaction = function(txid, options, callback) {
				callback(null, txid);
			};
			sinon.spy(runebased, '_getAddressDetailedTransaction');
			var txids = ['one', 'two', 'three', 'four'];
			runebased.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
			runebased.getAddressHistory('address', { from: 1, to: 3 }, function(err, data) {
				if (err) {
					return done(err);
				}
				runebased._normalizeAddressArg.callCount.should.equal(1);
				runebased._getAddressStrings.callCount.should.equal(1);
				runebased.getAddressTxids.callCount.should.equal(1);
				runebased._getAddressDetailedTransaction.callCount.should.equal(2);
				runebased._paginateTxids.callCount.should.equal(1);
	...
	describe('#getAddressSummary', function() {
		...
		it('will handle error from getAddressTxids', function(done) {
			...
			runebased.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			runebased.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
			var address = '';
			var options = {};
			runebased.getAddressSummary(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
		...
		it('will handle error from getAddressBalance', function(done) {
			...
			runebased.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
			runebased.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
			var address = '';
			var options = {};
			runebased.getAddressSummary(address, options, function(err) {
				should.exist(err);
		...
		it('should set all properties', function(done) {
			...
			runebased.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
			runebased.getAddressBalance = sinon.stub().callsArgWith(2, null, {
				received: 30 * 1e8,
				balance: 20 * 1e8
			});
			...
			var options = {};
			runebased.getAddressSummary(address, options, function(err, summary) {
				runebased._paginateTxids.callCount.should.equal(1);
				runebased._normalizeAddressArg.callCount.should.equal(1);
				runebased.getAddressTxids.callCount.should.equal(1);
				runebased.getAddressBalance.callCount.should.equal(1);
				runebased._paginateTxids.args[0][1].should.equal(0);
				runebased._paginateTxids.args[0][2].should.equal(1000);
		...
		it('will get from cache with noTxList', function(done) {
			...
			runebased.getAddressSummary(address, options, function(err, summary) {
				...
				runebased.getAddressSummary(address, options, function(err, summary) {
					checkSummary(summary);
					runebased._paginateTxids.callCount.should.equal(0);
					runebased._normalizeAddressArg.callCount.should.equal(2);
					runebased.getAddressTxids.callCount.should.equal(1);
					runebased.getAddressBalance.callCount.should.equal(1);
					done();
		...
		it('will skip querying the mempool with queryMempool set to false', function(done) {
			...
			runebased.getAddressSummary(address, options, function() {
				getAddressMempool.callCount.should.equal(0);
				runebased._paginateTxids.callCount.should.equal(1);
				runebased._normalizeAddressArg.callCount.should.equal(1);
				runebased.getAddressTxids.callCount.should.equal(1);
		...
		it('will give error from _paginateTxids', function(done) {
			...
			runebased._paginateTxids = sinon.stub().throws(new Error('test'));
			var address = 'qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR';
			var options = {
				queryMempool: false
			};
	...
	describe('#getRawBlock', function() {
		...
		it('will give rpc error from client getblockhash', function(done) {
			...
			runebased.getRawBlock(10, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give rcp error from client getblock', function(done) {
			...
			sinon.spy(runebased, '_tryAllClients');
			runebased.nodes.push({
				client: {
					getBlock: sinon.stub().callsArgWith(2, { code: -1, message: 'Test error' })
				}
			});
			runebased.getRawBlock(blockhash, function(err) {
				should.exist(err);
				runebased._tryAllClients.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		});
		it('will try all nodes for getblock', function(done) {
			...
			runebased.getRawBlock(blockhash, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
		...
		it('will get block by height', function(done) {
			...
			runebased.getRawBlock(0, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
	...
	describe('#getBlock', function() {
		...
		it('will give an rpc error from client getblock', function(done) {
			...
			runebased.getBlock(0, function(err) {
				runebased._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
		...
		it('will getblock as runebasecore object from height', function(done) {
			var runebased = new RunebaseService(baseConfig);
			sinon.spy(runebased, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			runebased.getBlock(0, function(err, block) {
				should.not.exist(err);
				runebased._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				...
				block.should.be.instanceof(runebasecore.Block);
				done();
			});
		});
		it('will getblock as runebasecore object', function(done) {
			var runebased = new RunebaseService(baseConfig);
			sinon.spy(runebased, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			runebased.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
				should.not.exist(err);
				runebased._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache', function(done) {
			...
			runebased.getBlock(hash, function(err, block) {
				should.not.exist(err);
				runebased._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache with height (but not height)', function(done) {
			...
			runebased.getBlock(0, function(err, block) {
				...
				runebased.getBlock(0, function(err, block) {
					should.not.exist(err);
					runebased._tryAllClients.callCount.should.equal(3);
	...
	describe('#getBlockHashesByTimestamp', function() {
		it('should give an rpc error', function(done) {
			...
			runebased.getBlockHashesByTimestamp(1517410000, 1517843954, function(err, hashes) {
				should.exist(err);
	...
	describe('#getBlockHeader', function() {
		...
		it('will give error from getBlockHash', function() {
			var runebased = new RunebaseService(baseConfig);
			sinon.spy(runebased, '_maybeGetBlockHash');
			var getBlockHash = sinon.stub().callsArgWith(1, { code: -1, message: 'Test error' });
			runebased.nodes.push({
				client: {
					getBlockHash: getBlockHash
			...
			runebased.getBlockHeader(10, function(err) {
				runebased._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockheader', function() {
			...
			runebased.getBlockHeader(blockhash, function(err) {
				runebased._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockhash', function() {
			...
			runebased.getBlockHeader(0, function(err) {
				runebased._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('will give result from client getblockheader (from height)', function() {
			...
			runebased.getBlockHeader(81419, function(err, blockHeader) {
				should.not.exist(err);
				runebased._maybeGetBlockHash.callCount.should.equal(1);
		...
		it('will give result from client getblockheader (from hash)', function() {
			...
			runebased.getBlockHeader(blockhash, function(err, blockHeader) {
				should.not.exist(err);
				runebased._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				blockHeader.should.deep.equal(result);
	...
	describe('#_maybeGetBlockHash', function() {
		it('will not get block hash with an address', function(done) {
			...
			runebased._maybeGetBlockHash('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX', function(err, hash) {
				if (err) {
					return done(err);
				}
				runebased._tryAllClients.callCount.should.equal(0);
				getBlockHash.callCount.should.equal(0);
		...
		it('will not get block hash with non zero-nine numeric string', function(done) {
			...
			runebased._maybeGetBlockHash('109a', function(err, hash) {
				if (err) {
					return done(err);
				}
				getBlockHash.callCount.should.equal(0);
		...
		it('will get the block hash if argument is a number', function(done) {
			...
			runebased._maybeGetBlockHash(10, function(err, hash) {
				if (err) {
					return done(err);
				}
	...
	describe('#getBlockOverview', function() {
		...
		it('will handle error from maybeGetBlockHash', function(done) {
			var runebased = new RunebaseService(baseConfig);
			sinon.spy(runebased, '_maybeGetBlockHash');
			runebased._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
			runebased.getBlockOverview(blockhash, function(err) {
				err.should.be.instanceOf(Error);
				runebased._maybeGetBlockHash.callCount.should.equal(1);
				done();
			});
		...
		it('will give expected result', function(done) {
			...
			runebased.getBlockOverview(blockhash, function(err, blockOverview) {
				if (err) {
					return done(err);
				}
				...
				runebased.getBlockOverview(blockhash, function(err, blockOverview) {
					checkBlock(blockOverview);
					runebased._maybeGetBlockHash.callCount.should.equal(2);
					getBlock.callCount.should.equal(1);
					done();
				});
			});
	...
	describe('#estimateFee', function() {
		it('will give rpc error', function(done) {
			...
			runebased.estimateFee(1, function(err) {
				should.exist(err);
		...
		it('will call client estimateFee and give result', function(done) {
			...
			runebased.estimateFee(1, function(err, feesPerKb) {
				if (err) {
					return done(err);
				}
				estimateFee.callCount.should.equal(1);
	...
	describe('#sendTransaction', function(done) {
		var tx = runebasecore.Transaction(txhex);
		it('will give rpc error', function() {
			...
			runebased.sendTransaction(txhex, function(err) {
				should.exist(err);
		...
		it('will send to client and get hash', function() {
			...
			runebased.sendTransaction(txhex, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('will send to client with absurd fees and get hash', function() {
			...
			runebased.sendTransaction(txhex, { allowAbsurdFees: true }, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('missing callback will throw error', function() {
			...
			var transaction = runebasecore.Transaction();
			(function() {
				runebased.sendTransaction(transaction);
			}).should.throw(Error);
			sendRawTransaction.callCount.should.equal(1);
		});
	});
	...
	describe('#getRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			runebased.getRawTransaction('txid', function(err) {
				should.exist(err);
				runebased._tryAllClients.callCount.should.equal(1);
		...
		it('will try all nodes', function(done) {
			...
			runebased.getRawTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
	...
	describe('#getTransaction', function() {
		it('will give rpc error', function(done) {
			...
			runebased.getTransaction('txid', function(err) {
				should.exist(err);
				runebased._tryAllClients.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will try all nodes', function(done) {
			...
			runebased.getTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
				...
				getRawTransaction.callCount.should.equal(1);
				runebased._tryAllClients.callCount.should.equal(1);
				tx.should.be.an.instanceof(runebasecore.Transaction);
				done();
			});
		});
		it('will get from cache', function(done) {
	...
	describe('#getDetailedTransaction', function() {
		...
		it('will give rpc error', function(done) {
			...
			runebased.getDetailedTransaction(txid, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				runebased._tryAllClients.callCount.should.equal(1);
				done();
		...
		it('should give a transaction with all properties', function(done) {
			...
			runebased.getDetailedTransaction(txid, function(err, tx) {
				if (err) {
					return done(err);
				}
				checkTx(tx);
	...
	describe('#getBestBlockHash', function() {
		it('will give rpc error', function(done) {
			...
			runebased.getBestBlockHash(function(err) {
				should.exist(err);
				getBestBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will call client getInfo and give result', function(done) {
			...
			runebased.getBestBlockHash(function(err, hash) {
				if (err) {
					return done(err);
				}
				getBestBlockHash.callCount.should.equal(1);
	...
	describe('#getSpentInfo', function() {
		it('will give rpc error', function(done) {
			...
			runebased.getSpentInfo({}, function(err) {
				should.exist(err);
		...
		it('will empty object when not found', function(done) {
			...
			runebased.getSpentInfo({}, function(err, info) {
				should.not.exist(err);
				getSpentInfo.callCount.should.equal(1);
	...
	describe('#getInfo', function() {
		it('will give rpc error', function(done) {
			...
			runebased.getInfo(function(err) {
				should.exist(err);
				err.should.be.an.instanceof(errors.RPCError);
				done();
			});
		...
		it('will call client getInfo and give result', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased.node.getNetworkName = sinon.stub().returns('testnet');
			...
			runebased.getSubsidy = sinon.stub().callsArgWith(1, null, 400000000);
			runebased.nodes.push({
				client: {
					getInfo: getInfo,
				},
			...
			runebased.getInfo(function(err, info) {
				if (err) {
					return done(err);
				}

				runebased.getSubsidy.callCount.should.equal(1);
				runebased.client.getInfo.callCount.should.equal(1);
	...
	describe('#generateBlock', function() {
		it('will give rpc error', function(done) {
			...
			runebased.generateBlock(10, function(err) {
				should.exist(err);
		...
		it('will call client generate and give result', function(done) {
			...
			runebased.generateBlock(10, function(err, hashes) {
				if (err) {
					return done(err);
				}
	...
	describe('#stop', function() {
		...
		it('will exit spawned process', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased.spawn = {};
			runebased.spawn.process = new EventEmitter();
			runebased.spawn.process.kill = sinon.stub();
			runebased.stop(done);
			runebased.spawn.process.kill.callCount.should.equal(1);
			runebased.spawn.process.kill.args[0][0].should.equal('SIGINT');
			runebased.spawn.process.emit('exit', 0);
		});
		it('will give error with non-zero exit status code', function(done) {
			...
			runebased.spawn.process.kill.callCount.should.equal(1);
			runebased.spawn.process.kill.args[0][0].should.equal('SIGINT');
			runebased.spawn.process.emit('exit', 1);
		});
		it('will stop after timeout', function(done) {
			var runebased = new RunebaseService(baseConfig);
			runebased.shutdownTimeout = 300;
			runebased.spawn = {};
			runebased.spawn.process = new EventEmitter();
			runebased.spawn.process.kill = sinon.stub();
			runebased.stop(function(err) {
	...
	describe('#getAddressesMempoolBalance', function() {
		it('will give rpc error', function(done) {
			...
			runebased.getAddressesMempoolBalance([], {}, function(err, result) {
				should.exist(err);
				runebased._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will calc mempool balance', function(done) {
			...
			var runebased = new RunebaseService(baseConfig);
			sinon.spy(runebased, '_normalizeAddressArg');
			sinon.spy(runebased, '_getBalanceFromMempool');
			var getAddressMempool = sinon.stub().callsArgWith(1, null, {
				result: deltas
			});
			runebased.nodes.push({
			...
			runebased.getAddressesMempoolBalance([], {}, function(err, result) {
				should.not.exist(err);
				runebased._getBalanceFromMempool.callCount.should.equal(1);
				runebased._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
	...
	describe('#getJsonRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			runebased.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err) {
				should.exist(err);
				runebased._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give transaction', function(done) {
			...
			runebased.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err, response) {
				should.not.exist(err);
				runebased._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
	...
	describe('#getSubsidy', function() {
		it('will give rpc error', function(done) {
			...
			runebased.getSubsidy(0, function(err) {
				should.exist(err);
		...
		it('will give subsidy', function(done) {
			...
			runebased.getSubsidy(0, function(err, result) {
				should.not.exist(err);
				getSubsidy.callCount.should.equal(1);
	...
	describe('#getJsonBlock', function() {
		it('will give rpc error from getBlockHash', function(done) {
			...
			runebased.getJsonBlock(0, function(err) {
				should.exist(err);
				runebased._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give rpc error from getBlock', function(done) {
			...
			runebased.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err) {
				should.exist(err);
				runebased._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give jsonBlock using block height', function(done) {
			...
			runebased.getJsonBlock(80000, function(err, result) {
				should.not.exist(err);
				runebased._tryAllClients.callCount.should.equal(2);
				runebased._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
		...
		it('will give jsonBlock using blockhash', function(done) {
			...
			runebased.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err, result) {
				should.not.exist(err);
				runebased._tryAllClients.callCount.should.equal(1);
				runebased._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
	...
	describe('#subscribeBalance', function() {
		...
		it('will not add an invalid address', function() {
			var runebased = new RunebaseService(baseConfig);
			var emitter = new EventEmitter();
			runebased.subscribeBalance(emitter, ['invalidaddress']);
			should.not.exist(runebased.subscriptions.balance['invalidaddress']);
			log.info.callCount.should.equal(1);
		});
		it('will add a valid address', function() {
			var runebased = new RunebaseService(baseConfig);
			var emitter = new EventEmitter();
			runebased.subscribeBalance(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(1);
		...
		it('will handle multiple address subscribers', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			runebased.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			runebased.subscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		...
		it('will not add the same emitter twice', function() {
			...
			runebased.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(2);
		});
	});
	...
	describe('#unsubscribeBalance', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			runebased.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		...
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			runebased.unsubscribeBalance(emitter1);
			runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			runebased.unsubscribeBalance(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			runebased.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
			log.info.callCount.should.equal(1);
		});
		it('will remove empty addresses', function() {
			var runebased = new RunebaseService(baseConfig);
			...
			runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			runebased.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			runebased.unsubscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(2);
		});
		it('will unsubscribe emitter for all addresses', function() {
			var runebased = new RunebaseService(baseConfig);
			...
			var emitter2 = new EventEmitter();
			runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			runebased.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(runebased, 'unsubscribeBalanceAll');
			runebased.unsubscribeBalance(emitter1);
			runebased.unsubscribeBalanceAll.callCount.should.equal(1);
			runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			runebased.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#unsubscribeBalanceAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			runebased.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			runebased.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			runebased.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			runebased.unsubscribeBalanceAll(emitter1);
			runebased.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			runebased.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			runebased.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(runebased.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#_notifyBalanceSubscribers', function() {
		...
		it('will notify balance subscribers', function(done) {
			...
			var runebased = new RunebaseService(baseConfig);
			sinon.spy(runebased, '_notifyBalanceSubscriber');
			runebased._getAddressesFromTransaction = sinon.stub().returns(addresses);
			runebased.getAddressSummary = sinon.stub().callsArgWith(2, null, {
			...
			emitter.on('berycoind/addressbalance', function(data) {
				...
				runebased._notifyBalanceSubscriber.callCount.should.equal(1);
				runebased._getAddressesFromTransaction.callCount.should.equal(1);
				emitter.emit.callCount.should.equal(1);
				done();
			});
			...
			runebased._notifyBalanceSubscribers('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', transaction);
	...
	describe('#_notifyBalanceSubscriber', function() {
		...
		it('will notify balance subscriber', function(done) {
			...
			emitter.on('berycoind/addressbalance', function(data) {
				...
				data.txid.should.equal('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
				emitter.emit.callCount.should.equal(1);
				runebased.getAddressSummary.callCount.should.equal(1);
				done();
			});
			...
			runebased._notifyBalanceSubscriber('qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z', '8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
	...
	describe('#listUnspent', function() {
		it('will give rpc error', function(done) {
			...
			runebased.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err) {
				should.exist(err);
		...
		it('will give array of unspent transaction outputs', function(done) {
			...
			runebased.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err, result) {
				should.not.exist(err);
				listUnspent.callCount.should.equal(1);
	...
	describe('#getNewAddress', function() {
		it('will give rpc error', function(done) {
			...
			runebased.getNewAddress(function(err, result) {
				should.exist(err);
	...
	describe('#callContract', function() {
		it('will give rpc error', function(done) {
			...
			runebased.callContract('f6177bc9812eeb531907621af6641a41133dea9e', 'd7bb99ba', {}, function(err, result) {
				should.exist(err);
	...
	describe('#getAccountInfo', function() {
		it('will give rpc error', function(done) {
			...
			runebased.getAccountInfo('f6177bc9812eeb531907621af6641a41133dea9e', function(err, result) {
				should.exist(err);
	...
	describe('#getTransactionReceipt', function() {
		it('will give rpc error', function(done) {
			...
			runebased.getTransactionReceipt('51b34cbcaf6fe3b687bf3d954f3baaeb377f5d0a3a8cdd29d899c049f6954a49', function(err, result) {
				should.exist(err);
				runebased._tryAllClients.callCount.should.equal(1);
	...
	describe('#getDgpInfo', function() {
		it('will give rpc error', function(done) {
			...
			runebased.getDgpInfo(function(err, result) {
				should.exist(err);
				runebased._tryAllClients.callCount.should.equal(1);
	...
	describe('#getMiningInfo', function() {
		it('will give rpc error', function(done) {
			...
			runebased.getMiningInfo(function(err, result) {
				should.exist(err);
				runebased._tryAllClients.callCount.should.equal(1);
	...
	describe('#getStakingInfo', function() {
		it('will give rpc error', function(done) {
			...
			runebased.getStakingInfo(function(err, result) {
				should.exist(err);
				runebased._tryAllClients.callCount.should.equal(1);
