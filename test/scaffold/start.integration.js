...
var RunebaseService = require('../../lib/services/runebased');
...
describe('#start', function() {
  ...
  describe('will dynamically create a node from a configuration', function() {
    ...
    it('require each runebasecore-node service with default config', function(done) {
      var node;
      var TestNode = function(options) {
        options.services[0].should.deep.equal({
          name: 'runebased',
          module: RunebaseService,
          config: {
            spawn: {
              datadir: './data'
            }
    ...
    it('require each runebasecore-node service with explicit config', function(done) {
      var node;
      var TestNode = function(options) {
        options.services[0].should.deep.equal({
          name: 'runebased',
